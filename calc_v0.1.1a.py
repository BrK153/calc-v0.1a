...
def excluir_caractere():
    display.config(state=tk.NORMAL)
    display.delete('end-2c')  # Apaga o último caractere digitado
    display.config(state=tk.DISABLED)

# Função para lidar com a porcentagem
def calcular_porcentagem():
    expressao = display.get(1.0, tk.END)
    try:
        # Extrair o número e a porcentagem da expressão
...
# Estilo dos botões
style = ttk.Style()
style.configure('Equal.TButton', font=('Arial', 14, 'bold'), background='#4CAF50', foreground='white')
...
style.configure('Delete.TButton', font=('Arial', 14, 'bold'), background='#F44336', foreground='white')
style.configure('Clear.TButton', font=('Arial', 14, 'bold'), background='#FF9800', foreground='white')
